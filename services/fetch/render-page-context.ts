  ...
  try {
    ...
    const extracted = await page.evaluate(() => {
      ...
      const headings = Array.from(document.querySelectorAll("h1, h2, h3"))
        .map((node) => node.textContent?.replace(/\s+/g, " ").trim() ?? node.getAttribute("aria-label")?.trim() ?? "")
        .filter(Boolean)
        .slice(0, 12);
    ...
    return withSignals(html, {
      url,
      title: cleanupText(extracted.title) || "제목을 찾지 못했습니다.",
      description: cleanupText(extracted.description) || "설명을 찾지 못했습니다.",
      ...
      textSnippet: cleanupText(extracted.text).slice(0, 3500) || "본문 텍스트를 충분히 추출하지 못했습니다.",
      textLength: cleanupText(extracted.text).length,
      source: "playwright",
      screenshotDataUrl,
    });
  } finally {
